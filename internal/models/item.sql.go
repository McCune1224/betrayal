...
const searchItemByDescription = `-- name: SearchItemByDescription :many
SELECT id, name, description, rarity, cost FROM item
WHERE LOWER(description) LIKE LOWER($1)
ORDER BY rarity DESC, name ASC
`

func (q *Queries) SearchItemByDescription(ctx context.Context, lower string) ([]Item, error) {
	rows, err := q.db.Query(ctx, searchItemByDescription, lower)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const searchItemByKeyword = `-- name: SearchItemByKeyword :many
SELECT id, name, description, rarity, cost FROM item
WHERE LOWER(name) LIKE LOWER($1) OR LOWER(description) LIKE LOWER($1)
ORDER BY rarity DESC, name ASC
`

func (q *Queries) SearchItemByKeyword(ctx context.Context, lower string) ([]Item, error) {
	rows, err := q.db.Query(ctx, searchItemByKeyword, lower)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
