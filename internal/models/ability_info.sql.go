...
const searchAbilityByDescription = `-- name: SearchAbilityByDescription :many
SELECT id, name, description, default_charges, any_ability, role_specific_id, rarity FROM ability_info
WHERE LOWER(description) LIKE LOWER($1)
ORDER BY rarity DESC, name ASC
`

func (q *Queries) SearchAbilityByDescription(ctx context.Context, lower string) ([]AbilityInfo, error) {
	rows, err := q.db.Query(ctx, searchAbilityByDescription, lower)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const searchAbilityByKeyword = `-- name: SearchAbilityByKeyword :many
SELECT id, name, description, default_charges, any_ability, role_specific_id, rarity FROM ability_info
WHERE LOWER(name) LIKE LOWER($1) OR LOWER(description) LIKE LOWER($1)
ORDER BY rarity DESC, name ASC
`

func (q *Queries) SearchAbilityByKeyword(ctx context.Context, lower string) ([]AbilityInfo, error) {
	rows, err := q.db.Query(ctx, searchAbilityByKeyword, lower)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
