...
<main class="min-h-screen bg-slate-950 p-6 text-slate-100"><div class="mx-auto max-w-5xl">
  ...
  {:else if data}<p class="mt-3 text-slate-400">Discord: {data.discord_connected ? 'connected' : 'disabled'} · {data.summary.configured} configured · {data.summary.missing} missing</p>
    <form class="mt-8 flex flex-wrap gap-3" onsubmit={update}><select name="kind" class="bg-slate-900 p-2"><option value="vote">Vote</option><option value="action">Action</option><option value="log">Command log</option><option value="admin">Admin</option><option value="lifeboard">Lifeboard</option></select>{#if resources}<select aria-label="Discord server" bind:value={selectedGuild} class="bg-slate-900 p-2">{#each resources.guilds as guild}<option value={guild.id}>{guild.name}</option>{/each}</select><select name="channel_id" required class="min-w-56 bg-slate-900 p-2"><option value="">Select Discord channel</option>{#each resources.channels.filter((channel) => channel.guild_id === selectedGuild) as channel}<option value={channel.id}>#{channel.name}{channel.category ? ` · ${channel.category}` : ''}</option>{/each}</select>{:else}<input name="channel_id" required placeholder="Channel ID (Discord unavailable)" class="bg-slate-900 p-2" />{/if}<input name="message_id" placeholder="Lifeboard message ID" class="bg-slate-900 p-2" /><button class="border border-emerald-400 px-4 py-2" disabled={busy}>Save channel</button></form>
