...
Still open (WT-4/6 scope, don't perpetuate): `tests/database/fuzzy_test.go`
reads `DATABASE_POOLER_URL` (prod-dangerous without the WT-4 guard),
`tests/inventory/suite_test.go` silently skips without a postgres socket,
`inv/create.go` still has placeholder embed copy ("Idk finished inventory lol"),
and `roll` `luckTable` is dead (unregistered).
